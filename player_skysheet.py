...
logger.info(f"Loaded {len(original_sheet)} notes, {len(time_groups)} frame-groups")
logger.info("First 5 frame-groups:")
for i, (t, keys) in enumerate(time_groups[:5]):
    logger.info(f"  Frame {frame}: {t:.3f}s: {keys}")
...
try:
    for time_pos, keys in time_groups:
        ...
        logger.info(f"[{time_pos:.3f}] Frame chord: {keys}")
        for key in keys:
