...
# 先生出時間差
for j in range(len(original_sheet)):
    if j == len(original_sheet) - 1:
        original_sheet[j]["time_area"] = 0
        break
    original_sheet[j]["time_area"] = original_sheet[j + 1]["time"] - original_sheet[j]["time"]
...
for note in original_sheet:
    ...
    # logger.info(f"channels = {pygame.mixer.get_num_channels()}")
